...
def create_arg_parser():
    ...
    pg_com_model.add_argument('-m', '--match-via-annotation', type=str,
                              help="the metabolite annotation type to use for matching exchange metabolites of different community members (e.g. metanetx.chemical)")

    pg_abundance = pg_com_model.add_mutually_exclusive_group()
    
    pg_abundance.add_argument('--equal-abd', action='store_true',
                              help="set abundances to be equal for all community members")
    
    pg_abundance.add_argument('--abd-file', type=str,
                              help="a comma separated file containing the input model file names and their abundance. No header should be used in the file.")

    pg_com_model.add_argument('--medium', type=str,
                              help="the medium to be used in the community model, as a comma separated file containing a column 'compounds' and a column 'maxFlux'.")
    ...
    pg_output.add_argument('--fva-flux', type=float,
                           help="run FVA on the exchange metabolites of the community model and store the flux vector in a file. Set the threshold of the objective that needs to be achieved.")

    pg_output.add_argument('--fba-interaction', action='store_true',
                           help="run FBA on the community model and store the flux of exchange metabolites and whether they are cross-fed in a file")

    pg_output.add_argument('--fva-interaction', type=float,
                           help="run FVA on the community model and store the flux of exchange metabolites and whether they are cross-fed in a file. Set the threshold of the objective that needs to be achieved.")
...
def check_args(args):
    ...
    if args.fva_interaction is not None:
